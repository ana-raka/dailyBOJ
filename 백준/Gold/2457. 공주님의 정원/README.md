...
메모리: 46228 KB, 시간: 376 ms

## 풀이 방법

그리디 문제로 사이즈가 12인 달력 배열을 생성 후 꽃이 피는 달을 기준으로 각 인덱스에 추가 해준다. 그 후 각 배열을 꽃이 지는 시간을 기준으로 내림 차순 정렬하여 탐색 시 맨 각 달의 첫번째 꽃만 확인하여 
최대가 되는 꽃을 계속 추가하는 식으로 구현하였다. 
